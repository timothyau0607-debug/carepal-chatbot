...
export function VoicePanel({
  ...
  const orbEl = (
    <div
      className={`carepal-orb shrink-0 ${compact ? "carepal-orb--compact" : ""}`}
  ...
  const micControls = (
    <div className={`flex flex-col items-center ${compact ? "gap-0.5" : "gap-2"}`}>
      {!stt.clientReady ? (
        <>
          <div
            className={`inline-flex items-center justify-center rounded-full bg-stone-200 text-stone-500 ${compact ? "h-8 w-8" : "h-14 w-14"}`}
            aria-hidden
          >
            <MicOff className={compact ? "size-4" : "size-6"} />
          </div>
          <span
            className={
              compact
                ? "text-center text-[0.6rem] leading-tight text-stone-400"
        ...
        <>
          <button
            ...
            className={`inline-flex items-center justify-center rounded-full bg-teal-600 text-white shadow-md shadow-teal-600/25 transition hover:bg-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-1 ${compact ? "h-8 w-8" : "h-14 w-14 shadow-lg shadow-teal-600/30 focus:ring-offset-2"}`}
            aria-pressed={stt.sessionOpen}
          >
            {stt.sessionOpen ? (
              <Mic className={compact ? "size-4" : "size-6"} />
            ) : (
              <MicOff className={compact ? "size-4" : "size-6"} />
            )}
          </button>
          <span
            className={
              compact
                ? "max-w-[11rem] text-center text-[0.6rem] leading-tight text-stone-500"
  ...
  const micBlock = compact ? (
    <>
      {stt.interim && (
        <p className="max-w-full truncate rounded border border-dashed border-teal-300/70 bg-white/60 px-1.5 py-0.5 text-center text-[0.65rem] leading-tight text-stone-600">
          辨識中：{stt.interim}
        </p>
      )}
      {sttError && (
        <p className="text-center text-[0.65rem] leading-tight text-rose-600" role="status">
          {sttError}
        </p>
      )}
