...
- **staff_interaction_satisfaction**：**與醫護人員互動是否大致滿意、有無困惑**等（**僅**使用者有表達的；**沒有則必須空字串**。**不可**填「尚未提及」「本輪未說」等佔位句；佔位與留空相同，由系統處理）。
- **for_staff**：轉述給醫護的今日摘要，三個子欄都須**只依使用者**：
  ...
  - **praise_for_staff**：若**使用者有**稱讚、感謝**醫師/護理/醫療團隊**，或**對院方、櫃台、整體就醫服務／環境的正面評價**，摘錄此處；**若沒有就留空字串。嚴禁**把小晴的客套、鼓勵語當成使用者的讚美**。

## 合併
- 你會看到「既有」各欄與**既有今日給醫護摘要**；去重、合併。無新內容可保留前稿微調贅字。
...
小晴：${assistantText.slice(0, MAX_IN_ASST)}`.trim();
  ...
  const satForDb = isPlaceholderStaffSatisfaction(next.sat) ? "" : next.sat;
