...
const STAFF_PRAISE_TIMING_MIN = 1;
const STAFF_PRAISE_TIMING_MAX = 4;

/**
 * 醫護端第 1～4 則訊息左右：提醒主動轉述今日家屬／病友的感謝、讚美、好評（需先有非空今日回饋區）。
 */
export function buildStaffPraiseTimingHint(params: {
  ...
  const phase = `你是醫護端的第 ${uc} 則發言（在第 ${STAFF_PRAISE_TIMING_MIN}～${STAFF_PRAISE_TIMING_MAX} 則區間內）。`;
  if (uc === 1 && !q) {
    return `【醫護對談節奏—優先正向回饋】${phase}對方本則偏寒暄、問候或僅閒聊、**未見明確提問**時：**務必**先以**一至二句口語**，依「今日家屬／病友回饋」主動轉述病友／家屬的**感謝、讚美或對醫護／院方的好評**（不必等對方先問），再輕輕承接；遵守【轉述方式】。**嚴禁**杜撰、**僅**依摘錄。`;
  }
  if (q) {
