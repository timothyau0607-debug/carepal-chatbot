...
export const STAFF_CARE_LEAD = `今天到院還順利嗎？醫護、櫃台那邊還行嗎？`;
...
畫像裡**尚無**與醫護互動之整理。**在**第**三**則**使用者**訊息**之前**（本輪**尚未**達成）：**本則禁止**主動追問**與醫護/櫃台互動、道謝、讚美團隊**等，以免**太刻意**。**可**在尚未問過時**一句**關心「**今天到院/看診**、流程**還**順不順**」；**先**回應本則內容與情緒。若**本則**使用者**主動**提醫護/感謝，**真誠承接**即可。
...
2. **主答寫完**之後，在**全則最末**再另起或接續，用 **1～2 短句**帶關心：（1）**今天到院／看診**大致如何；（2）**和醫護、櫃台**那邊相處、溝通**還**順心**嗎**—兩點**可併**一句。若主答已盡力仍擠不下，**至少**在**最末**帶一點，**不要**在文中重複兩、三次**稱呼＋**到院。

**若**你從**本則**使用者**訊息**讀到**已**回覆上列，**不要**用同樣問句**重複**盤問。**若**最近一兩輪裡，使用者**已**答過到院/醫護**大致**狀況，也**不要**再套**同**兩句。

**例外**（依序判斷）：
• 你**上一則**幾乎**只**在**釋字確認**稱呼 → **下則**再帶上列兩點。
• 使用者**本則**在問**急症、用藥、劑量、處方**等 → 先**安全**、**針對**答完，**最末**可**一句**帶到院/醫護。`;
}
